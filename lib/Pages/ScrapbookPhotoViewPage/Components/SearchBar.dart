...
class _ScrapbookPhotoViewSearchBarState
  ...
  Widget _renderTextField() {
    Widget innerTextField = TextField(
        ...
        style: TextStyle(
            color: _selected ? EmbarkGray : EmbarkLightGray,
    ...
    if (!_selected) {
      ...
      return Container(
        ...
        IconButton(icon: Icon(Icons.arrow_back, size: 16, color: EmbarkGray)),
        Flexible(child: innerTextField),
        IconButton(icon: Icon(Icons.clear, size: 16, color: EmbarkGray),padding: EdgeInsets.only(right:0))
      ]));
    }
  }
